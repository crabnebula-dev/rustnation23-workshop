use tauri_sys::window::{WebviewWindowBuilder, PhysicalSize, TitleBarStyle};
use yew::prelude::*;

#[function_component]
pub fn AddChannel() -> Html {
    let onclick = Callback::from(move |_| {
        log::debug!("opening input window");

        wasm_bindgen_futures::spawn_local(async {
            WebviewWindowBuilder::new("add-channel")
                .set_url("/add-channel")
                .set_size(PhysicalSize::new(400, 250))
                .set_title_bar_style(TitleBarStyle::Overlay)
                .set_title("Add Channel")
                .set_resizable(false)
                .build()
                .await
                .unwrap();
    ...
    html! {
        <li key="add" class="channel-preview">
            <button {onclick}>
